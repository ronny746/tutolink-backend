...
//       select: "question options correctAnswer explanation" // Populate full question
//     });
...
exports.getQuizReview = async (req, res) => {
  try {
    const { quizId } = req.params; // URL se quizId le rahe hain
    const userId = req.user.id; // Token se userId le rahe hain

    let user = await User.findById(userId);
    if (!user) return res.status(404).json({ message: "User not found" });

    // ✅ Find user's quiz attempt
    const quizAttempt = user.quizzesTaken.find(q => q.quizId.toString() === quizId);
    if (!quizAttempt) return res.status(404).json({ message: "Quiz not found in user's history" });

    // ✅ Fetch quiz details
    let quiz = await Quiz.findById(quizId).select("name duration totalQuestions questions");

    if (!quiz) return res.status(404).json({ message: "Quiz not found" });

    // ✅ Attach explanation to each question
    let review = quiz.questions.map(q => {
      let userAnswer = quizAttempt.answers.find(ans => ans.questionId.toString() === q._id.toString());
      return {
        question: q.question,
        options: q.options,
        selectedOption: userAnswer ? userAnswer.selectedOption : null,
        correctAnswer: q.correctAnswer,
        explanation: q.explanation || "No explanation available" // Ensure explanation exists
      };
    });

...
exports.getHome = async (req, res) => {
  try {
    ...
    const [subjects, mysliders, featuredQuizzes, allQuizzes, latestContent, user, topRankedUsers] = await Promise.all([
      ...
      Slider.find().sort({ createdAt: -1 }), // Newest first
      ...
      Content.find({}, "title pdfUrl videoUrl").sort({ createdAt: -1 }), // Newest first
    ...
    res.json({
      ...
      data: {
        mysliders, // 🟢 **Sliders**
        subjects: subjectsWithColors, // 🟢 **Subjects**
        ...
        topRankedUsers: rankedUsers // 🟢 **Top Ranked Users**
      }
    });
  } catch (error) {

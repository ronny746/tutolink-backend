...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    // Y-position anchor
    const startY = doc.y;
    const circleX = doc.page.width - 100;
    const circleY = startY + 40;
    const radius = 30;

    // Draw summary text on the left
    doc
      .font('Helvetica-Bold')
      .fontSize(16)
      .fillColor('#000')
      .text(`Name: ${user.name}`, 40, startY)
      .text(`Subject: ${quizDetail.name}`)
      .text(`Accuracy: ${((correctAnswers / totalQuestions) * 100).toFixed(2)}%`);

    // Draw score circle on right
    doc
      .save()
    ...
    doc
      ...
      .fontSize(14)
      .fillColor('#000')
      .text(`${obtainedPoints}/${totalPoints}`, circleX - 20, circleY - 8, {
        width: 40,
        align: 'center',
      })
      .restore();

    doc.moveDown(3); // Add vertical space after the section

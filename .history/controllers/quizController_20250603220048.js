...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    // Title
    doc.fontSize(22).fillColor('#000000').text(`Quiz Review: ${quizDetail.name}`, { align: 'center' });
    doc.moveDown(1.5);
    ...
    review.forEach((item, idx) => {
      ...
      // Question
      doc.fontSize(16).fillColor('#111').text(`${questionNum} ${item.question}`);
      doc.moveDown(0.5);

      // Options
      item.options.forEach((opt, i) => {
        const label = String.fromCharCode(65 + i);
        const isSelected = label === item.selectedOption;
        const isRight = label === item.correctAnswer;

        let prefix = '-';
        if (isRight) prefix = '[Correct]';
        else if (isSelected) prefix = '[Wrong]';

        let color = '#000';
        if (isRight) color = '#28a745'; // green
        else if (isSelected) color = '#dc3545'; // red

        doc.fontSize(13).fillColor(color).text(`${prefix} ${label}) ${opt}`);
      });
      ...
      doc
        .fontSize(11)
        .fillColor('#000')
        .text(`• Your Answer: ${item.selectedOption || 'Not Answered'}`)
      ...
      // Divider
      doc.moveDown(1.2);
      doc.lineWidth(0.5).moveTo(40, doc.y).lineTo(doc.page.width - 40, doc.y).strokeColor('#ddd').stroke();
      doc.moveDown(1);
    });

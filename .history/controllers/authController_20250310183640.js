...
exports.getUserPerformance = async (req, res) => {
  try {
    ...
    // ✅ Remove quizzes that no longer exist
    const validQuizzes = user.quizzesTaken.filter((quiz) => quiz.quizId); // ✅ Only keep existing quizzes
    if (validQuizzes.length !== user.quizzesTaken.length) {
      // ✅ Update the user's `quizzesTaken` field
      await User.findByIdAndUpdate(userId, { quizzesTaken: validQuizzes });
      user.quizzesTaken = validQuizzes;
    }
    ...
    const subjects = validQuizzes.reduce((acc, quiz) => {
    ...
    const recentQuizzes = validQuizzes
    ...
    const responseData = {
      points: user.points,
      quizzesPlayed: validQuizzes.length,
      rank: `#${rank}`,

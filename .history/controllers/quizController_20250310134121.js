...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    answers.forEach((submittedAnswer) => {
      ...
      if (!correctQuestion) {
        console.log("Question not found in quiz:", submittedAnswer.questionId);
      }
    ...
    if (quizIndex !== -1) {
      ...
      // ✅ Safe score update
      if (existingQuiz.score !== undefined) {
      ...
      existingQuiz.answers.forEach((existingAnswer, index) => {
        ...
        if (newAnswer) {
          existingQuiz.answers[index] = { ...existingQuiz.answers[index], ...newAnswer };
        }
      });

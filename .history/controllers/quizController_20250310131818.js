...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    answers.forEach((submittedAnswer) => {
      let correctQuestion = quiz.questions.find(
        (q) => q._id.toString() === submittedAnswer.questionId
      );

      if (correctQuestion && correctQuestion.correctAnswer === submittedAnswer.selectedOption) {
        newScore += 1; // ✅ +1 for each correct answer
      }
    });
    ...
    if (quizIndex !== -1) {
      ...
      existingQuiz.answers.forEach((existingAnswer, index) => {
        let newAnswer = answers.find(ans => ans.questionId.toString() === existingAnswer.questionId);
        if (newAnswer) {
          existingQuiz.answers[index] = newAnswer;
        }
      });

      answers.forEach(newAnswer => {
        let exists = existingQuiz.answers.some(ans => ans.questionId.toString() === newAnswer.questionId);
        if (!exists) {
          existingQuiz.answers.push(newAnswer);
        }
      });

      // ✅ Adjust user points correctly
      ...
      user.quizzesTaken.push({ quizId, score: newScore, answers });
    }

    user.points += newScore; // ✅ Update total points correctly
    await user.save();

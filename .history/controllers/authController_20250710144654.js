...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find()
      .select('name points selectedSubjects quizzesTaken classOrCourseId') // only needed fields
    ...
    const formattedUsers = users.map((user) => ({
      ...
      subjects: user.selectedSubjects?.length
        ? user.selectedSubjects.map((s) => s?.name || 'Unnamed Subject')
        : ['N/A'],
      classOrCourses: user.classOrCourseId?.length
        ? user.classOrCourseId.map((c) => c?.name || 'Unnamed Course')
        : ['N/A'],
    }));

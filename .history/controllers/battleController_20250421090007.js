...
exports.submitAnswer = async (req, res) => {
  try {
    ...
    // Find or create score record for the user
    let userScore = battle.scores.find(s => s.userId.toString() === userId);
    if (!userScore) {
      userScore = { userId, score: 0, answers: [] };
      battle.scores.push(userScore);
    }

    userScore.score += points;
    if (!userScore.answers) userScore.answers = [];
    userScore.answers.push({ questionId, answer, isCorrect });

    await battle.save();
    res.status(200).json({ success: true, message: "Answer submitted", score: userScore.score });
  } catch (error) {
...
exports.getLiveScores = async (req, res) => {
  try {
    const { battleId } = req.query;
    const battle = await Battle.findById(battleId, "player1Score player2Score");
    if (!battle) {
      return res.status(404).json({ message: "Battle not found" });
    }
    res.status(200).json({

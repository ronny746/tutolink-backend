...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    doc.fontSize(20).text(`Quiz Review - ${quizDetail.name}`, { align: 'center' });
    doc.moveDown();

    review.forEach((item, idx) => {
      doc.fontSize(14).text(`${idx + 1}. ${item.question}`);
      item.options.forEach((opt, i) => {
        const optionLabel = String.fromCharCode(65 + i); // A, B, C, ...
        doc.text(`   ${optionLabel}) ${opt}`);
      });

      doc.text(`✅ Selected: ${item.selectedOption?.selectedOption || "Not Answered"}`);
      doc.text(`✅ Correct Answer: ${item.correctAnswer}`);
      doc.text(`💡 Explanation: ${item.explanation}`);
      doc.moveDown();
    });

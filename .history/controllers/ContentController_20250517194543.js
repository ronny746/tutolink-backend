...
exports.getHome = async (req, res) => {
  try {
    ...
    const [subjects, mysliders, featuredQuizzes, allQuizzes, latestContent, topRankedUsers] = await Promise.all([
      Subject.find({ classOrCourseId: user.classOrCourseId }, "name description imageUrl"), // 🟢 Filtered by user's category
      ...
      Content.find({ categoryId: user.categoryId }, "title pdfUrl videoUrl").sort({ createdAt: -1 }),

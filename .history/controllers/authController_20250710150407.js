...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find()
      .select('name points quizzesTaken categoryId classOrCourseId')
      .populate('categoryId', 'title') // only get title
      .populate('classOrCourseId', 'name') // only get name
      .lean();
    ...
    const formattedUsers = users.map((user) => ({
      ...
      quizzesPlayed: Array.isArray(user.quizzesTaken) ? user.quizzesTaken.length : 0,
      category: user.categoryId?.title || 'N/A',

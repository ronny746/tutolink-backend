...
exports.askAI = async (req, res) => {
  try {
    ...
    if (!question || typeof question !== "string" || question.trim().length < 3 || question.trim().length > 1000) {
    ...
    if (!ctx) {
      // 2) Fallback to web
      prompt = null; // will be set in fallback call
      source = "web";
    } else {
    ...
    let raw;
    if (source === "web") {
      raw = await getWebAnswerFallback(q);
    } else {

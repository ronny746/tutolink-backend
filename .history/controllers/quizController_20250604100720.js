...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    // ✅ Step 1: Create blurred logo buffer with canvas
    const logoPath = path.join(__dirname, '../assets/logo.png');
    const logo = await loadImage(logoPath);
    const canvas = createCanvas(200, 200);
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, 200, 200);
    ctx.globalAlpha = 0.07; // Transparency for watermark effect
    ctx.drawImage(logo, 0, 0, 200, 200);
    const blurredLogoBuffer = canvas.toBuffer('image/png');

    // ✅ Step 2: Start PDF creation
    ...
    const drawWatermark = () => {
      ...
      doc.image(blurredLogoBuffer, centerX, centerY, { width: 200 });
    };

    drawWatermark(); // on first page
    doc.on('pageAdded', () => drawWatermark()); // for other pages

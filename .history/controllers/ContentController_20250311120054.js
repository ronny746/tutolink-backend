...
exports.getHome = async (req, res) => {
  try {
    ...
    // ✅ Fetch top 3 ranked users based on max score
    topRankedUsers = await User.find({}, "name points avatar")
      .sort({ score: -1 }); // Highest score first

    topRankedUsers = topRankedUsers.map(user => ({
      ...user._doc,
      name: user._id.toString() === userId ? "You" : user.name
    }));
    const allUsers = await User.find({}, "name points").sort({ points: -1 });
    const userIndex = allUsers.findIndex(user => user._id.toString() === userId);
    const userRank = userIndex !== -1 ? userIndex + 1 : null; // Rank starts from 1
    const userPoints = userIndex !== -1 ? allUsers[userIndex].points : 0;

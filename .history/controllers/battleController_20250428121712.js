...
exports.getAllBattles = async (req, res) => {
  try {
    const battles = await Battle.find()
      .populate('quizId', 'title')  // Populate quiz title (Maths Basics, etc.)
      .populate('createdBy', 'username')  // Populate creator's username (Rohit)
      .populate('participants', '_id')  // Just count participants, no need to populate all fields
    ...
    const battleDetails = battles.map(battle => {
      return {
        title: battle.quizId ? battle.quizId.title : 'N/A',
        code: battle.battleCode,
        time: battle.startTime ? `Starts in ${Math.max(0, Math.floor((new Date(battle.startTime) - new Date()) / (1000 * 60)))} mins` : 'N/A',
        creator: battle.createdBy ? battle.createdBy.username : 'Unknown',
        participants: battle.participants.length,
        joined: battle.participants.some(participant => participant._id.toString() === req.user._id.toString()),  // Check if current user is part of the battle
      };
    });

...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    let review = quiz.questions.map((q, index) => ({
      ...
      question: q.question,
      options: q.options,
      selectedOption: takenQuiz.answers ? takenQuiz.answers[index] : null,
    ...
    const filePath = path.join(__dirname, `../public/reviews/review_${quizId}_${userId}.pdf`);
    const writeStream = fs.createWriteStream(filePath);
    doc.pipe(writeStream);

    // 🧠 Title
    doc.fontSize(22).fillColor('#333').text(`📘 Quiz Review: ${quizDetail.name}`, { align: 'center' });
    doc.moveDown();

    // ✍️ Iterate through questions
    review.forEach((item, index) => {
      const selected = item.selectedOption?.selectedOption;
      ...
      doc
        .fontSize(16)
        .fillColor('#000')
        .text(`${index + 1}. ${item.question}`, { continued: false, underline: true });
      ...
      item.options.forEach((opt, i) => {
        ...
        if (label === correct) color = '#28a745'; // green
        if (label === selected && selected !== correct) color = '#dc3545'; // red

        doc.fontSize(13).fillColor(color).text(`   ${label}) ${opt}`);
      });
      ...
      doc
        .fontSize(12)
        .fillColor(isCorrect ? '#28a745' : '#dc3545')
        .text(isCorrect ? '✅ Correct Answer!' : '❌ Wrong Answer');

      doc.fillColor('#000').text(`• Your Answer: ${selected || "Not Answered"}`);
      doc.text(`• Correct Answer: ${correct}`);
      ...
      doc.moveDown(1.5);
      doc.moveTo(doc.x, doc.y).lineTo(doc.page.width - 40, doc.y).stroke('#cccccc'); // Divider line
      doc.moveDown(1);
    });
    ...
    writeStream.on('finish', () => {
      res.download(filePath, `quiz-review-${quizId}.pdf`);
    });

  } catch (error) {
    console.error("Quiz Review PDF Error:", error);
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};

...
exports.getBattleDetails = async (req, res) => {
  try {
    const { battleId, battleCode } = req.query;

    if (!battleId && !battleCode) {
      return res.status(400).json({ message: "Please provide either battleId or battleCode" });
    }

    const query = battleId
      ? { _id: battleId }
      : { battleCode: battleCode.toUpperCase() }; // optional: normalize code
...
exports.getAllBattles = async (req, res) => {
  try {
    const battles = await Battle.find()
      .populate({
        path: 'quizId',
        select: 'name subjectId duration',
        populate: {
          path: 'subjectId',
          select: 'name',
        },
      })
      .populate('createdBy', 'name email') // or 'username' if that's your field
      .populate('participants', 'name email')
      .populate('winner', 'name email')
      .populate('winners', 'name email')
      .populate('scores.userId', 'name email');

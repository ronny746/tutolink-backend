  ...
  try {
    ...
    const user = await User.findById(userId)
      .populate({
        ...
        select: "title totalQuestions subject"
      })
      .populate({
    ...
    // ✅ Rank Calculation (Users with Higher Points)
    const rank = await User.countDocuments({ points: { $gt: user.points } }) + 1;

    // ✅ Subject-wise Total Quizzes & Points
    const subjects = user.quizzesTaken.reduce((acc, quiz) => {
      if (!quiz.quizId) return acc; // Skip if quiz is null
      const subject = quiz.quizId.subjectId || "Unknown"; // Handle missing subjects
      if (!acc[subject]) acc[subject] = { totalQuizzes: 0, totalPoints: 0 };
      acc[subject].totalQuizzes += 1;
      acc[subject].totalPoints += quiz.totalPoints || 0; // Handle missing totalPoints
      return acc;
    }, {});

    // ✅ Recent Quizzes Played (Latest 5)
    const recentQuizzes = user.quizzesTaken
      .filter((quiz) => quiz.quizId) // Skip if quiz is null
      .sort((a, b) => new Date(b.dateTaken) - new Date(a.dateTaken))
      .slice(0, 5)
      .map((quiz) => {
        const questionsAnswered = quiz.answers.length;
        const isCompleted = questionsAnswered === quiz.quizId.totalQuestions;
        ...
        return {
          ...
          quizTime: quiz.quizTime || "N/A",
          status: isCompleted ? "Check Result" : "Continue Quiz",
        };
      });

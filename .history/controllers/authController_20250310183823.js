...
exports.getUserPerformance = async (req, res) => {
  try {
    ...
    // ✅ Fetch all valid quiz IDs from the Quiz model
    const validQuizIds = new Set(await Quiz.distinct("_id"));

    // ✅ Filter out quizzes that no longer exist
    const updatedQuizzesTaken = user.quizzesTaken.filter((quiz) => 
      quiz.quizId && validQuizIds.has(quiz.quizId._id.toString())
    );

    // ✅ If any quizzes were removed, update the user document
    if (updatedQuizzesTaken.length !== user.quizzesTaken.length) {
      await User.findByIdAndUpdate(userId, { quizzesTaken: updatedQuizzesTaken });
      user.quizzesTaken = updatedQuizzesTaken; // ✅ Update local reference too
    }
    ...
    const subjects = updatedQuizzesTaken.reduce((acc, quiz) => {
      if (!quiz.quizId.subjectId) return acc;
      const subject = quiz.quizId.subjectId.name || "Unknown";
    ...
    const responseData = {
      points: user.points,
      quizzesPlayed: updatedQuizzesTaken.length,
      rank: `#${rank}`,

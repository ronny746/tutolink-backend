...
exports.getHome = async (req, res) => {
  try {
    ...
    // }));
    const attemptedQuizIds = await featuredQuizzes.map(quiz.id, { userId: user.quizzesTaken. });

// 🧹 Filter out attempted quizzes from featuredQuizzes
const unattemptedFeaturedQuizzes = featuredQuizzes.filter(
  quiz => !attemptedQuizIds.includes(quiz._id.toString())
);

// 🟢 Featured Quizzes with Colors (unattempted only)
const quizzesWithColors = unattemptedFeaturedQuizzes.map((quiz, index) => ({
  ...quiz._doc,
  color1: colors[(index + subjects.length) % colors.length].color1,
  color2: colors[(index + subjects.length) % colors.length].color2
}));
    ...
    const ongoingQuizzes = allQuizzes.slice(0, 10);

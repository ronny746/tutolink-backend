...
exports.getAdminDashboard = async (req, res) => {
  try {
    ...
    const [totalUsers, totalSubjects, totalQuizzes, totalCourses, liveQuizzes, newUsersThisWeek, mostActiveQuizDoc, topUsers] = await Promise.all([
      User.countDocuments(),
      ...
      Quiz.findOne().sort({ participants: -1 }), // most participated
      User.find().sort({ averageScore: -1 }).limit(3), // top performers
    ]);
    ...
    res.status(200).json({
      ...
      data: {
        stats: [
          { label: 'Total Users', value: totalUsers, page: 0, icon: 'people' },
          { label: 'Total Subjects', value: totalSubjects, page: 1, icon: 'menu_book' },
        ...
        newUsersThisWeek,
        mostActiveQuiz: mostActiveQuizDoc?.name || 'N/A',
        topPerformers,
      }
    });
  } catch (error) {

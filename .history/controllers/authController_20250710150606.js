...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find()
      .select('name points quizzesTaken categoryId classOrCourseId') // select only required
    ...
    const formattedUsers = users.map((user) => ({
      ...
      category: user.categoryId && typeof user.categoryId === 'object'
        ? user.categoryId.title
        : 'N/A',
      classOrCourses: Array.isArray(user.classOrCourseId) && user.classOrCourseId.length

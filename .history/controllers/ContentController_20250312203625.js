...
exports.getHome = async (req, res) => {
  try {
    ...
    const allQuizzes = await Quiz.find().sort({ startTime: 1 }); 

    // Manually filter upcoming quizzes
    const upcomingQuizzes = allQuizzes.filter(quiz => {
      return new Date(quiz.startTime) > currentTime;
    });
    
    // Limit to 10
    const limitedUpcomingQuizzes = upcomingQuizzes.slice(0, 10);
    const ongoingQuizzes = await Quiz.find({

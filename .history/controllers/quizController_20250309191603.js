...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    const userId = req.user.id; // Token se user ID extract kar rahe hain
    ...
    if (quizIndex !== -1) {
      // ✅ Already attempted → Old score remove kar ke new add karo
      let oldScore = user.quizzesTaken[quizIndex].score || 0; // Handle undefined case
      user.points -= oldScore; // Old score remove
    ...
    user.points += score; // ✅ Ensure correct points update
    await user.save();

    res.json({ message: "Quiz submitted successfully", points: user.points, user });
  } catch (error) {

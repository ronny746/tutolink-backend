...
exports.completeBattle = async (req, res) => {
  try {
    ...
    const sortedScores = battle.scores.sort((a, b) => b.score - a.score);
    battle.winner = sortedScores[0]?.userId || null;

    await battle.save();
    res.status(200).json({ success: true, message: "Battle completed", winner: battle.winner });
  } catch (error) {
...
exports.getBattleHistory = async (req, res) => {
  try {
    const userId = '67ceb5a70b945bea8798670a'; // Replace with `req.user._id` if using auth middleware

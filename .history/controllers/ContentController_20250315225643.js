...
const apiKey = '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';
const ai = new genAI.GenerativeModel({ model: "gemini-pro", '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' });
...
exports.getHome = async (req, res) => {
  try {
    ...
    const [subjects, mysliders, featuredQuizzes, allQuizzes, latestContent, user, topRankedUsers] = await Promise.all([
      ...
      Quiz.find({}, "name totalQuestions duration rating startTime instructions ").sort({ rating: -1 }), // Highest-rated quizzes
      ...
      User.findById(userId, "name points dailyScore lastCheckIn"),
...
exports.openAiQuestions = async (req, res) => {
  try {
    ...
    const response = await ai.generateContent(`Create a multiple-choice question about ${topic}.`);
    res.json({ question: response.candidates[0].content });

...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    // Step 1: Load logo and blur it using canvas
    const logoPath = path.join(__dirname, '../assets/logo.png');
    const logoImg = await loadImage(logoPath);
    const canvas = createCanvas(200, 200);
    const ctx = canvas.getContext('2d');

    // Apply blur using shadow trick
    ctx.filter = 'blur(6px)';
    ctx.drawImage(logoImg, 0, 0, 200, 200);
    const blurredLogoBuffer = canvas.toBuffer();

    // Step 2: Create PDF and insert the blurred logo
    ...
    const drawWatermark = () => {
      ...
      doc.image(blurredLogoBuffer, centerX, centerY, {
        width: 200,
        opacity: 0.03
      });
    };

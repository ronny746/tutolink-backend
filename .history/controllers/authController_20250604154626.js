...
exports.getUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId, " -bookmarkedQuizzes -favoriteSubjects")
      .populate({ path: "categoryId", select: "name" })
      .populate({ path: "classOrCourseId", select: "name" }).populate({ path: "classOrCourseId", select: "name" });;
    ...
    // ✅ Convert to object and attach quizzesTaken length
    const userObj = user.toObject();
    userObj.quizzesTakenCount = user.quizzesTaken?.length || 0;

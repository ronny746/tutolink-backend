...
exports.getQuizReview = async (req, res) => {
  try {
    ...
    const logoPath = path.join(__dirname, '../assets/logo.png'); // update path to your actual logo
    ...
    const drawWatermark = () => {
      ...
      doc.image(logoPath, centerX, centerY, {
        width: 200,
        opacity: 0.05 // just to keep it faint, optional
      });
    };
    // ✅ Draw logo on first page
    drawWatermark();

    // ✅ Also apply on every new page
    doc.on('pageAdded', () => {
      drawWatermark();
    });
